...
namespace FK.BLL.Interfaces
{
    public interface IUserFridgeService
    {
        ...
        void Delete(UserFridge fridgeProduct);  //удалить холодос от юзера
        ...
        UserFridge GetUserFridgeByFridgeIdUserId(int? fridge_id, int? user_id);  //получить связь конкретного юзера с конкретным холодосом по параметрам
